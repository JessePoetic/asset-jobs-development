                ...
                <div className="filter-checkboxes w-form">
                  <label className="w-checkbox filter-checkbox-wrapper">
                    ...
                    <input
                      ...
                      id="checkbox"
                      name="checkbox"
                      checked={showCorporate && showProperty}
                      onChange={setSelectedType}
                      value="all"
                      style={{ opacity: 0, position: "absolute", zIndex: -1 }}
                    />
                    <span className="filter-checkbox-label w-form-label">
                      All
                    </span>
                  </label>
                  <label className="w-checkbox filter-checkbox-wrapper">
                  ...
                  <label className="w-checkbox filter-checkbox-wrapper">
                    ...
                    <input
                      ...
                      checked={showProperty && !showCorporate}
                      onChange={setSelectedType}
                      value="property"
    ...
    return (
      <React.Fragment>
        {data.map((item) => {
          ...
          const title = item.Title.toLowerCase().replace(/\b\w/g, (l) =>
            _.capitalize(l)
          );
...
class App extends React.Component {
  ...
  fetchListings = () => {
    ...
    return Promise.all([
      // Asset Residential
      axios
        .get(`${baseUrl}?id=ba2a034d-57c4-4b52-9ab0-c8247cd7e5ed`, {})
        .then(({ data }) => {
      ...
      // Shelton Residential
      axios
        .get(`${baseUrl}?id=dbd73b24-6a77-40d3-8660-18bc932a28c6`, {})
        .then(({ data }) => {
          const res = _.get(data, "ResultList", []);
          return res.map((obj) => ({
            ...obj,
      ...
      // ABRES Corporate
      axios
        .get(`${baseUrl}?id=b361be71-34f2-478d-a14c-5df623c0086b`, {})
        .then(({ data }) => {
          const res = _.get(data, "ResultList", []);
          return res.map((obj) => ({
            ...obj,
      ...
      // Shelton Corporate
      axios
        .get(`${baseUrl}?id=71085fbf-a0c2-42b0-9923-eff62b48d38e`, {})
        .then(({ data }) => {
          const res = _.get(data, "ResultList", []);
          return res.map((obj) => ({
            ...obj,
      ...
      axios
        .get(`${baseUrl}?id=febc91d1-934a-46fe-8395-2a4d3be6f086`, {})
        .then(({ data }) => {
          const res = _.get(data, "ResultList", []);
          return res.map((obj) => ({
            ...obj,
  ...
  async componentDidMount() {
    ...
    const states = _.sortBy(
      _.uniq(this.allJobListings.map(({ State }) => State))
    );
